...
export default function CompanyValues() {
  return (
    <section className="v-container relative">
      ...
      <ul className="v-stagger grid gap-y-20 md:gap-y-8 lg:grid-cols-3 lg:gap-x-7.5 lg:gap-y-0">
        {companyValues.map(({ name, illustration, description }) => (
          ...
          >
            <img
              ...
              loading="lazy"
              className="v-iris size-50.5 shrink-0"
            />

            <div className="v-rise">
              <h2 className="text-label ps-(--text-label--letter-spacing) font-medium uppercase md:ps-0 lg:ps-(--text-label--letter-spacing)">
                {name}
              </h2>
