...
export default function ProjectGrid({ projects }: ProjectGridProps) {
  return (
    <section aria-labelledby="projects-heading" className="v-container">
      ...
      <ul className="v-stagger grid gap-y-10 md:gap-y-8 lg:grid-cols-3 lg:gap-x-7.5">
        {projects.map(({ name, description, image }, index) => (
          <li
            key={name}
            className="bg-peach-pale rounded-card v-wipe flex flex-col overflow-clip md:flex-row lg:flex-col"
          >
            <img
              src={image.src}
              alt=""
            ...
            <div className="flex flex-col justify-center p-8 text-center md:w-87.5 md:shrink-0 lg:w-auto">
              <h3 className="text-label text-peach-ink ps-(--text-label--letter-spacing) font-medium uppercase">
                {name}
              </h3>
